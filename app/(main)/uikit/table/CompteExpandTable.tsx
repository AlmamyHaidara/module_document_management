...
import { Client, CompteClient, TypeDocument } from '@/types/types';

interface PropsType {
    comptesClient: CompteClient[];
    findCompte: () => Promise<void>;
}

const ComptExpendTable = ({ comptesClient, findCompte }: PropsType) => {
    ...
    const [comptes, setComptes] = useState<CompteClient[]>(comptesClient);
    ...
    useEffect(() => {
        ...
        // ProductService.getProductsWithOrdersSmall().then((data) => setComptes(data));
        initFilters1();
    }, []);
    ...
    const deleteProduct = () => {
        let _products = (comptes as any)?.filter((val: any) => val.id !== product.id);
        setComptes(_products);
        setDeleteProductDialog(false);
        setProduct(emptyProduct);
        toast.current?.show({
            severity: 'success',
    ...
    const findIndexById = (id: string) => {
        let index = -1;
        for (let i = 0; i < (comptes as any)?.length; i++) {
            if ((comptes as any)[i].id === id) {
                index = i;
                break;
            }
        }
    ...
    const deleteSelectedProducts = () => {
        let _products = (comptes as any)?.filter((val: any) => !(selectedProducts as any)?.includes(val));
        setComptes(_products);
        setDeleteProductsDialog(false);
    ...
    const expandAll = () => {
        ...
        comptes.forEach((p) => (_expandedRows[`${p.id}`] = true));
    ...
    const searchBodyTemplate = (compte: CompteClient) => {
        return <Button icon="pi pi-search" onClick={() => findCompte()} />;
    };
    ...
    const rowExpansionTemplate = (data:  CompteClient & TypeDocument[]) => {
        console.log(data)
        return (
            <div className="orders-subtable">
                 ...
                 <DataTable value={data.TypesDocuments} responsiveLayout="scroll">
                     <Column field="id" header="Id" sortable></Column>
                     <Column field="code" header="Matricule" sortable></Column>
                     <Column field="nom_type" header="Numero Compte" sortable></Column>
        ...
        <div className="grid">
            <div className="col-12">
                <div className="card">
                    ...
                    <DataTable value={comptes} expandedRows={expandedRows} rowExpansionTemplate={rowExpansionTemplate} onRowToggle={(e) => setExpandedRows(e.data)} responsiveLayout="scroll"  dataKey="id" header={header}>
                        <Column expander style={{ width: '3em' }} />
                        <Column field="id" header="ID" sortable />
                        <Column field="matricule" header="Matricule" sortable />
                        <Column field="numero_compte" header="Numero Compte" sortable />
                        <Column field="type_compte" header="Type Compte" sortable />
                    ...
                    <Dialog visible={deleteProductsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteProductsDialogFooter} onHide={hideDeleteProductsDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {product && <span>Are you sure you want to delete the selected comptes?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
...
export default ComptExpendTable;
